...
LuaLaTeXText is an extension for Inkscape_ that allows adding
...
LuaLaTeXText is based on TexText written by Pauli Virtanen <pav@iki.fi>.
...
.. note::
   LuaLaTeXText requires Pdflatex and one of the following
     - Pdf2svg_
...
TEXTEXT_NS = u"http://www.iki.fi/pav/software/LuaLaTeXText/"
...
if USE_GTK:
    ...
    class AskText(object):
        """GUI for editing LuaLaTeXText objects"""
        ...
        def ask(self, callback):
            ...
            window.set_title("LuaLaTeX Text")
        ...
        def cb_ok(self, widget, data=None):
            ...
            self.text = buf.get_text(buf.get_start_iter(), buf.get_end_iter())
...
class LuaLaTeXText(inkex.Effect):
    ...
    def get_old(self):
        ...
        LuaLaTeXText-generated objects.
...
class Settings(object):
    ...
    def __init__(self):
        ...
        if USE_WINDOWS:
            self.keyname = r"Software\LuaLaTeXText\LuaLaTeXText"
        else:
...
class LatexConverterBase(object):
    ...
    def tex_to_pdf(self, latex_text, preamble_file):
        ...
        exec_command(['lualatex', self.tmp('tex')] + latexOpts)
        if not os.path.exists(self.tmp('pdf')):
            raise RuntimeError("lualatex didn't produce output")
